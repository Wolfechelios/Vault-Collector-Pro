...
export default function App() {
  ...
  return <div className="app-shell">
    <aside className="sidebar"><div className="brand"><div className="mark">V</div><div><h1>Vault</h1><span>Platform 0.6</span></div></div><nav>{nav('dashboard', '◆ Dashboard')}{nav('inventory', '▦ Inventory', items.length)}{nav('capture', '◉ Capture')}{nav('marketplace', '◇ Value & Sell')}{nav('storage', '⌖ Storage', storage.length)}{nav('import', '⇧ Import Center')}{nav('backup', '⇩ Backup Center')}</nav><div className="side-footer">VALUATION + MARKETPLACE<br/><strong>Private · Local-first</strong></div></aside>
    <main className="content"><header><div><p className="eyebrow">PROFESSIONAL ASSET PLATFORM</p><h2>{section === 'marketplace' ? 'Valuation & Marketplace' : section[0].toUpperCase() + section.slice(1)}</h2><p>Local catalog, capture intelligence, historical valuation and marketplace-ready records.</p></div><div className="header-actions"><button className="secondary" onClick={exportBackup}>Back up</button><button className="primary" onClick={() => setEditing(null)}>＋ Add item</button></div></header>{error && <div className="banner error">{error}</div>}
